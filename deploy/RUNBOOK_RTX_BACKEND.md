...
- HTTPS/Let's Encrypt временно не чиним. Caddy работает в HTTP-only режиме.
...
curl -I http://llm.luxriot.systems/    -> HTTP/1.1 200 OK
```
...
Для временного HTTP deploy важно оставить:

```bash
LUXRIOT_COOKIE_SECURE=0
```

Когда вернем HTTPS, поменять на `1`.
...
```bash
curl http://llm.luxriot.systems/health
curl -sS -H 'Content-Type: application/json' \
  -d '{"query":"installation prerequisites","k":3}' \
  http://llm.luxriot.systems/docs/search | head -c 2000
echo
```
...
Internet/ACME    -> VPS 178.104.125.49:80/tcp и 443/tcp для Let's Encrypt
```
...
Контекст: VPS face уже поднят на http://llm.luxriot.systems/ в HTTP-only режиме, backend еще не поднят.
...
Не трогай HTTPS, это отдаём админу.
```
